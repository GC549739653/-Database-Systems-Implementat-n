...
Bool isSubset(Bits b1, Bits b2)
{
	...
	for (int i = b1->nbytes-1; i >= 0; i--){
		if((b1-> bitstring[i] & (~b2-> bitstring[i])) != 0)
			return FALSE;
	}
	return TRUE; 
}
...
{ 
	// Byte *addr = addrInPage(p,pos,sizeof(Count)*2 + (b -> nbytes)+ sizeof(Count)*2 );
	Byte *addr = addrInPage(p,pos,b -> nbytes);
	memcpy(&(b -> bitstring[0]), addr, b -> nbytes);
}
...
void putBits(Page p, Offset pos, Bits b)
{
	// Byte *addr = addrInPage(p,pos,sizeof(Count)*2 + (b -> nbytes)+ sizeof(Count)*2 );
	Byte *addr = addrInPage(p,pos,b -> nbytes);
	memcpy(addr,&(b -> bitstring[0]),b -> nbytes);
}
